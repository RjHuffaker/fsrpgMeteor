...
angular.module('freedomsworn').factory('DeckUtils', ['$rootScope', 'CoreVars', 'PanelUtils', 'setPanelPosition',
	...
	service.addToFront = function(deck, newPanel){
		...
		PanelUtils.setAdjacentHorizontal(newPanel, firstPanel);
	...
	service.addToDeck = function(deck, newPanel, previousPanel){
		...
		if(previousPanel){
			...
			if(PanelUtils.hasNext(previousPanel)){
				...
				nextPanel.left.adjacent = newPanel._id;
			}
		} else {
			prevPanel = PanelUtils.getLast(deck.cardList);
		}
		
		newPanel.left.adjacent = prevPanel._id;
		prevPanel.right.adjacent = newPanel._id;
