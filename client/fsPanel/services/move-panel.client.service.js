...
angular.module('freedomsworn').factory('movePanel', ['$rootScope', 'CoreVars', 'PanelUtils', 'setPanelPosition',
	function($rootScope, CoreVars, PanelUtils, setPanelPosition){
		...
		return function(cardList, slot, panel, nearest, moveX, moveY){
			...
			// STEP 1: Check to see if either panel or slot has x_overlap,
			// If so, basic horizontal switch is the only available option.
			...
			// STEP 2: Check to see if panel is crossing ABOVE upper edge of slot.
			// If so, panel stack pushes slot down.
			...
			// STEP 3: Check to see if panel is crossing BELOW lower edge of slot.
			// If so, relocate to below slot.
